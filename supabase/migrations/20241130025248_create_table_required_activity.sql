-- Create required_activity table
...
CREATE INDEX idx_required_activity_ipcr_id ON required_activity(ipcr_id);

-- Add updated_at trigger
CREATE TRIGGER set_updated_at
    BEFORE UPDATE ON required_activity
    FOR EACH ROW
    EXECUTE FUNCTION fn_set_updated_at();

-- Enable Row Level Security
ALTER TABLE required_activity ENABLE ROW LEVEL SECURITY;

-- Create RLS policies
CREATE POLICY "Users can perform all operations on their own required activities"
    ON required_activity
    TO authenticated
    USING (ipcr_id IN (
