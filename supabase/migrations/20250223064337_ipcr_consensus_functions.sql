...
RETURNS BOOLEAN AS $$
BEGIN
    RETURN p_status IN ('revision_raw', 'reviewed_raw', 'revision');
END;
...
RETURNS BOOLEAN AS $$
BEGIN
    RETURN p_status IN ('under_review_raw', 'under_review');
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Function to get the appropriate status based on supervisor statuses
...
BEGIN
    ...
    FROM ipcr_immediate_supervisor
    WHERE ipcr_id = p_ipcr_id 
    AND is_immediate_sync_status(status)
    LIMIT 1;
    ...
    -- Check consensus for action-required statuses
    FOR v_consensus_status IN 
        SELECT unnest(enum_range(NULL::ipcr_supervisor_status))
    LOOP
        -- Skip if status doesn't require consensus
    ...
    -- For reviewed_raw specifically, maintain revision_raw until all supervisors reach reviewed_raw
    IF v_current_status = 'revision_raw'::ipcr_status THEN
        SELECT COUNT(*) = v_supervisor_count INTO v_has_consensus
        FROM ipcr_immediate_supervisor
        WHERE ipcr_id = p_ipcr_id 
        AND status = 'reviewed_raw';
