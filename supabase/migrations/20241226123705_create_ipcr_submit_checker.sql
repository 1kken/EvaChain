...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        result := ROW(false, 'IPCR must have at least one function')::validation_result;
        RETURN result;
    END IF;
    -- Check if each function has at least one indicator
    SELECT 
        f.id,
        f.title
    ...
    LIMIT 1;
    IF FOUND THEN
        result := ROW(false, format('Function "%s": No indicators found at any level', invalid_function.title))::validation_result;
        RETURN result;
    END IF;
    ...
    SELECT 
        CASE
            ...
            WHEN i.op_activity_indicator_id IS NULL THEN 'Missing Operational Plan Activity'
    ...
    AND (
        ...
        i.op_activity_indicator_id IS NULL
    )
    LIMIT 1;
    IF FOUND THEN
        result := ROW(false, format('In %s: %s', 
                              invalid_indicator.parent_name,
                              invalid_indicator.missing_field))::validation_result;
        RETURN result;
    END IF;
    ...
    LIMIT 1;
    IF FOUND THEN
        result := ROW(false, format('Direct indicator under function "%s": %s', 
                              invalid_indicator.function_title,
                              invalid_indicator.missing_field))::validation_result;
        RETURN result;
    END IF;
    -- If we get here, everything is valid
    result := ROW(true, 'IPCR is complete and valid')::validation_result;
    RETURN result;
END;
$$;
-- Grant necessary permissions
GRANT EXECUTE ON FUNCTION validate_ipcr(UUID) TO authenticated;