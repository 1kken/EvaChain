-- Create instructional_imperative table
CREATE TABLE instructional_imperative (
...
CREATE INDEX idx_instructional_imperative_ipcr_id ON instructional_imperative(ipcr_id);

-- Add updated_at trigger
CREATE TRIGGER set_updated_at
    BEFORE UPDATE ON instructional_imperative
    FOR EACH ROW
    EXECUTE FUNCTION fn_set_updated_at();

-- Enable Row Level Security
ALTER TABLE instructional_imperative ENABLE ROW LEVEL SECURITY;

-- Create RLS policies
CREATE POLICY "Users can perform all operations on their own instructional imperatives"
    ON instructional_imperative
    TO authenticated
    USING (ipcr_id IN (
