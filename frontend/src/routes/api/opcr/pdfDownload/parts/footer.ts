...
export function generateFooter(opcr: Tables<'opcr'>): Content {
...
export function generateAssesorSignatureBlock(opcr: Tables<'opcr'>): Content {
	return {
		columns: [
			createSignatureBlockBottom(
				'Assesed by',
				opcr.administrative_officer,
				'Administrative Officer'
			),
			createSignatureBlockBottom('Assesed by', opcr.planning_officer, 'Planning Officer'),
			createSignatureBlockBottom('Assesed by', opcr.human_resource, 'HR Officer'),
			createSignatureBlockBottom('Approved by', opcr.review_by, opcr.reviewer_position)
		],
		columnGap: 10,
		//horizontal,vertical
		margin: [0, 5]
...
function createSignatureBlockBottom(
	...
	const context: Content = {
		...
		table: {
			...
			body: [
				[{ text: `${head}:`, colSpan: 4, alignment: 'left' }, {}, {}, {}],
				[
					{ text: '' },
					{
						text: ''
