...
import { fetchOPCRById, fetchProfile } from './helper';
...
import { generateBottomSignature } from './parts/bottom_sig';
...
export async function generatePDF(supabase: SupabaseClient<Database>, opId: string): Promise<Blob> {
	...
	const opcr = await fetchOPCRById(supabase, opId);

	const profile = await fetchProfile(supabase, opcr.owner_id);
	...
	const docDefinition: TDocumentDefinitions = {
		...
		content: [
			//ulo ulo hahahahaha
			...generateHeader(logoBase64, opcr, profile.profile),
			generateSubHeader(profile.profile),
			createSignatureBlock(opcr.review_by, opcr.reviewer_position),
			await generateBody(opcr, supabase),
			generateFooter(opcr),
			generateAssesorSignatureBlock(opcr),
			generateBottomSignature(profile.profile)
		],
