<script lang="ts">
	...
	let { academicOffices, currentAcademicAccomplishmentAnalysis } = getHeadsChartStore();

	let selection = $academicOffices || [];
	let value = $state($currentAcademicAccomplishmentAnalysis);
...
<Select.Root
	...
	onValueChange={($event) => {
		value = $event;
		currentAcademicAccomplishmentAnalysis.set(value);
	}}
>
