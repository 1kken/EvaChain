<script lang="ts">
	...
	import { markIndicatorDoneSchema } from '../../utils/schemas/indicator_schema';
	...
	function onFileSelectAndHover(files: File[]) {
		...
		// Get the FileList from DataTransfer
...
<Dialog.Root bind:open={isOpen}>
	...
	<Dialog.Content class="w-11/12 sm:max-w-md">
		<Dialog.Header>
			...
			<Dialog.Description>
				Target indicator: {indicator.indicator}
			</Dialog.Description>
		</Dialog.Header>
		<form
			method="POST"
			action="?/markindicatordone"
			use:enhance
			class="space-y-6"
