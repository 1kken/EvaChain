<script lang="ts">
	...
	import { fetchIndicatorsByParam, fetchSubSupportFunctions } from '../../../utils/fetching_utils';
	import {
	...
	let { supportFunction }: { supportFunction: Tables<'support_function'> } = $props();

	// Types
	type DndItem =
		| (Tables<'sub_support_function'> & { itemType: 'sub_function' })
	...
	// Initialize stores
	const store = setSubSupportFunctionStore();
	...
	async function fetchData(): Promise<void> {
		try {
			...
			const [subFunctionsResult, indicatorsResult] = await Promise.all([
				fetchSubSupportFunctions(supportFunction.id),
				fetchIndicatorsByParam({
					url_params: 'support_function_id',
					id: supportFunction.id
				})
			]);
			...
			// Update stores
			$currentSubSupportFunctions = subFunctionsResult.data;
	...
	async function handlePositionsUpdate(updatedItems: DndItem[]): Promise<void> {
		const subFunctions = updatedItems.filter(
			(item): item is Tables<'sub_support_function'> & { itemType: 'sub_function' } =>
		...
		await Promise.all([
			updateSubSupportFunctionPositions(subFunctions),
			updateIndicatorPositions(indicators)
		]);

		$currentSubSupportFunctions = subFunctions;
		$currentIndicators = indicators;
	}

	$effect(() => {
		if (!isLoading) {
			const subFunctionItems = $currentSubSupportFunctions.map((item) => ({
				...item,
				itemType: 'sub_function' as const
			}));
...
<div class="rounded-lg border">
	<div class="flex min-h-[4rem] items-center justify-between p-4">
		<div class="flex items-center gap-2">
			...
			<div>
				<h3 class="text-sm font-semibold md:text-base">{supportFunction.name}</h3>
				<p class="text-muted-foreground text-xs md:text-sm">
					({!supportFunction.unit ? ' _' : supportFunction.unit} units)
				</p>
			</div>
		</div>
		<div class="flex gap-4">
			{#if $canEdit}
				{#snippet deleteAction()}
					<DeleteActionSupportFunction supportFunctionId={supportFunction.id} bind:isDrawerOpen />
				{/snippet}
				{#snippet updateDialog()}
					<SupportFunctionUpdateDialog supportFunctionId={supportFunction.id} bind:isDrawerOpen />
				{/snippet}
				{#snippet createSubSupportFunction()}
					<SubSupportFunctionCreateDialog
						subSupportId={supportFunction.id}
				...
				{#snippet createIndicatorDialog()}
					<CreateIndicatorDialog
						isDirectChild={true}
						config={{ type: 'support_function', id: supportFunction.id }}
						bind:isDrawerOpen={isAddDrawerOpen}
					/>
				{/snippet}
				<DropDownWrapper
