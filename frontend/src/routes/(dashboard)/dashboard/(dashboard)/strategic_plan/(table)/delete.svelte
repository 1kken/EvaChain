<script lang="ts">
	...
	const { removePlan, currentPlans } = getStrategicPlanStore();
	...
	const form = superForm(deleteForm, {
		...
		onUpdate({ form, result }) {
			...
			if (form.valid && action.stratPlan) {
				...
				removePlan(stratPlan.id);
				showWarningToast(`Succesfully deleted Strategic Plan ${stratPlan.title}`);
				closeAllTabs();
			}
		}
	});
	...
	const currentStratPlan = $currentPlans.find((plans) => plans.id === id);
	$formData.id = id.toString();
	$formData.expectedText = currentStratPlan?.title ?? 'error';
...
<AlertDialog.Root bind:open={isOpen}>
	...
	<AlertDialog.Content>
		...
		<form method="POST" action="?/deletestrategicplan" use:enhance>
