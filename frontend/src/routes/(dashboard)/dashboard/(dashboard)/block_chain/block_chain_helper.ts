...
export class IPFSFileTrackerLogRetriever {
	...
	async getAllFileActionLogs(
		...
		return events.sort((a, b) => {
			// Convert to numbers for comparison if they're BigNumber objects
			const timestampA =
				typeof a.timestamp === 'object' && 'toNumber' in a.timestamp
					? a.timestamp.toNumber()
					: Number(a.timestamp);
			const timestampB =
				typeof b.timestamp === 'object' && 'toNumber' in b.timestamp
