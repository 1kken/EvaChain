<script lang="ts">
	...
	import { createOpcrIndicatorSchema } from '../../../schema/indicator_schema';
	import type { OPCRIndicatorFormResult } from '../../../utils/types';

	//props
	interface Iprops {
		opcrFunctionId?: string;
		opcrCategoryId?: string;
	...
	//stores
	const { currentOpcrIndicators, addOpcrIndicator, size } = getOpcrIndicatorStore();
	const { createForm } = getOpcrIndicatorFormContext();
	...
	const form = superForm(createForm, {
		...
		dataType: 'json',
		validators: zodClient(createOpcrIndicatorSchema),
		...
		onUpdate({ form, result }) {
			...
			if (form.valid && action.opcrIndicator) {
				...
				addOpcrIndicator(opcrIndicator);
				showSuccessToast(`Succesfully added OPCR indicator `);
				isOpen = false;
				isDrawerOpen = false;
				if (isAddDrawerOpen) {
				...
				reset({
					data: {
						opcr_function_id: opcrFunctionId ?? null,
						opcr_function_category_id: opcrCategoryId ?? null,
						position: $size + 1
					},
					newState: {
						opcr_function_id: opcrFunctionId ?? null,
	...
	if (opcrFunctionId) {
		...
		$formData.opcr_function_category_id = null;
	}

	if (opcrCategoryId) {
		$formData.opcr_function_category_id = opcrCategoryId;
		//set to null the others
		$formData.opcr_function_id = null;
	}
...
<Dialog.Root bind:open={isOpen}>
	...
	<Dialog.Content class="max-h-[85vh] overflow-y-auto sm:max-w-[800px]">
		...
		<form action="?/createindicator" method="POST" use:enhance class="space-y-6">
			...
			{#if $formData.opcr_function_category_id}
				<input
					hidden
					name="dpcr_function_category_id"
					value={$formData.opcr_function_category_id}
				/>
			{:else}
			...
			<div class=" grid gap-2 md:grid-cols-2">
				<Form.Field {form} name="alloted_budget">
					<Form.Control>
						{#snippet children({ props })}
							<Form.Label>Alloted Budget</Form.Label>
							<Input
