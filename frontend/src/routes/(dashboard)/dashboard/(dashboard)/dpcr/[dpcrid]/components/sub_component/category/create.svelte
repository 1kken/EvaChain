<script lang="ts">
	...
	import { createDpcrCategorySchema } from '../../../schema/category_schema';
	...
	let { dpcrFunctionId, isDrawerOpen = $bindable() }: Iprops = $props();
	//stores
	const { createForm } = getDpcrCategoryFormContext();
	const { currentDpcrCategories, addDpcrCategory, size } = getDpcrCategoryStore();
	...
	const form = superForm(createForm, {
		...
		dataType: 'json',
		validators: zodClient(createDpcrCategorySchema),
		...
		onUpdate({ form, result }) {
			...
			if (form.valid && action.dpcrCategory) {
				...
				addDpcrCategory(dpcrCategory);
				showSuccessToast(`Succesfully added dpcr  category ${dpcrCategory.category}`);
				isOpen = false;
				isDrawerOpen = false;
				reset({
					data: { dpcr_function_id: dpcrFunctionId, position: $size + 1 },
					newState: { dpcr_function_id: dpcrFunctionId, position: $size + 1 }
				});
			}
		}
	});
...
<Dialog.Root bind:open={isOpen}>
	...
	<Dialog.Content class="max-h-[85vh] overflow-y-auto sm:max-w-[800px]">
		...
		<form action="?/createdpcrcategory" method="POST" use:enhance class="space-y-6">
