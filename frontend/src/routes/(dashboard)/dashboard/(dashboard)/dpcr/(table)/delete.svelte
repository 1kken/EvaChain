<script lang="ts">
	...
	const { removeDpcr, currentDpcrs } = getDpcrStore();
	...
	const form = superForm(deleteForm, {
		...
		onUpdate({ form, result }) {
			...
			if (form.valid && action.dpcr) {
				...
				removeDpcr(dpcr.id);
				showWarningToast(`Succesfully deleted DPCR ${dpcr.title}`);
				closeAllTabs();
			}
		}
	});
	...
	$formData.expectedText = currentDpcr?.title ?? 'error';
...
<AlertDialog.Root bind:open={isOpen}>
	...
	<AlertDialog.Content>
		...
		<form method="POST" action="?/deletedpcr" use:enhance>
