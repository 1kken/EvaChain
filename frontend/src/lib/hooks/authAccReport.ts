...
export const handleAccReportAccess: Handle = async ({ event, resolve }) => {
	// Only apply checks to the operational plan routes
	if (event.url.pathname.startsWith('/dashboard/accomplishment_report')) {
		// Only check permissions for GET requests (not for form actions/POST)
		if (event.request.method === 'GET') {
			// Check all permissions in parallel
			const hasPermissionCreateOpcr = await event.locals.hasPermission(
				'create_accomplishment_report'
			);

			console.log('hasPermissionCreateOpcr', hasPermissionCreateOpcr);
