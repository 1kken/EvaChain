...
export async function fetchTechAdminAccomplishmentPerformance(
	...
	// Fetch tech/admin office accomplishment data for the unit
	const { data, error } = await supabase.rpc('tech_admin_accomplishment_performance', {
		p_unit_id: profile.unit_id
	});

	if (error) {
		console.error('Error fetching tech/admin accomplishment performance data:', error);
		return null;
	}
