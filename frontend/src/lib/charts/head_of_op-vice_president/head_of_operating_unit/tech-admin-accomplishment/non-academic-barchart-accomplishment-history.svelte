<script lang="ts">
	...
	let { currentNonAcademicAccomplishmentAnalysis } = getHeadsChartStore();
	...
	function createChart(theme: string | undefined) {
		...
		// Use different colors for tech/admin - using blue tones instead of orange
		const barColor = isDark ? 'rgb(65, 149, 186)' : 'rgb(39, 117, 182)'; // Blue
		const barBackgroundColor = isDark ? 'rgba(65, 149, 186, 0.7)' : 'rgba(39, 117, 182, 0.7)';
	...
	$effect(() => {
		if ($currentNonAcademicAccomplishmentAnalysis) {
			fetchAccomplishmentReportData($currentNonAcademicAccomplishmentAnalysis).then(() => {
				createChart($mode);
			});
		}
	});
...
{:else if !$currentNonAcademicAccomplishmentAnalysis}
...
{:else}
	<canvas id="tech-admin-accomplishment-chart" bind:this={ctx}></canvas>
{/if}
